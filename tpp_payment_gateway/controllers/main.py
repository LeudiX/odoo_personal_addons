...
class PaymentTppController(http.Controller):
    _return_url = "/payment/tpp/return"
    _information_url = "/payment/tpp/information"
    ...
    def tpp__checkout2(self, **data):
        ...
        # if status === OK
        if status == "OK":
            data_dict = data_dict["data"]  # Ej: {'id': 383663, 'reference': 'S00047'}
            data_id = data_dict["id"]  # 383663
            data_reference = data_dict["reference"]  # 'S00047'
            _logger.info(
                "status: %s, id: %s, reference: %s", status, data_id, data_reference
            )
            tx_sudo = (
                request.env["payment.transaction"]
                .sudo()
                ._get_tx_from_notification_data("tpp", request.httprequest.data)
            )
