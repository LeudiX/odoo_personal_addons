...
class PaymentTransaction(models.Model):
    ...
    def execute_payment(self):
        ...
        _logger.info("Mostrando country:\n%s", self.partner_id.country_id.code)
        ...
        payload = {
            ...
            "amount": round(
                amount * 100, 2
            ),  # float(f"{self.amount}00"), #str(self.amount)+"00",
        ...
        _logger.info("La URL corta obtenida es")
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        _logger.info("notification_data_dict: %s", notification_data_dict)
        ...
        clientsecret = (
            self.env["payment.provider"].search([("code", "=", "tpp")]).client_secret
        )
        bankOrderCode = notification_data_dict["data"]["bankOrderCode"]
        originalCurrencyAmount = notification_data_dict["data"][
            "originalCurrencyAmount"
        ]
        # Concatenar los valores
        data = "{}{}{}{}".format(
            bankOrderCode, client_id, clientsecret, originalCurrencyAmount
        )

        # Calcular la firma utilizando SHA256
        signature = hashlib.sha256(data.encode()).hexdigest()
        _logger.info("Signature: {}".format(signature))
        _logger.info(
            "Firma remota: {}, Firma local: {}".format(
                notification_data_dict["data"]["signaturev2"], signature
            )
        )
        reference = notification_data_dict["data"]["reference"]
        if signature != notification_data_dict["data"]["signaturev2"]:
            raise ValidationError("tpp: " + _("Invalid Signature %s.", reference))
        _logger.info("reference: %s", reference)
        tx = self.search([("reference", "=", reference), ("provider_code", "=", "tpp")])
        if not tx:
            raise ValidationError(
                "tpp: " + _("No transaction found matching reference %s.", reference)
            )
        return tx
    ...
    def login(self):
        ...
        client_id = (
            self.env["payment.provider"].search([("code", "=", "tpp")]).client_id
        )
        client_secret = (
            self.env["payment.provider"].search([("code", "=", "tpp")]).client_secret
        )
        ...
        data = response.json()
        _logger.info("******DATOS DE AUTENTICACION EN TROPIPAY*****")
        _logger.info(data)
